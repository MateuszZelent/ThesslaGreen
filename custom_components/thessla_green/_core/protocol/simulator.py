...
class SimulatedAirPackTransport:
    ...
    def __init__(
        ...
        *,
        ...
        if len(serial_number) != 6 or any(not 0 <= int(part) <= 0xFFFF for part in serial_number):
        ...
        self._input_registers: dict[int, int] = {
            ...
            4: int(firmware[2]),
            **{
                16 + index: _encode_temperature((temperatures or {}).get(name, default))
            ...
            **{24 + index: int(value) for index, value in enumerate(serial_number)},
        }
    ...
    async def write_holding_registers(
        ...
        if any(
            address + offset not in self._holding_registers for offset in range(len(normalized))
        ):
    ...
    def _read_block(registers: Mapping[int, int], address: int, count: int) -> tuple[int, ...]:
        ...
        try:
            ...
            raise ReadResponseError(f"simulator does not expose register {exc.args[0]}") from exc
