...
4. Confirm the discovered model, firmware, Modbus endpoint/unit ID and the serial ports visible
   on the gateway host.
5. After setup, open the **Thessla Green** sidebar panel. It embeds the same `/ui/` animation and
   control panel as the standalone gateway dashboard.
6. Expose only the resulting fan and sensor entities that you want to Google Assistant.
...
The sidebar panel is an embedded gateway UI; it does not open Modbus from Home Assistant. If the
gateway uses an API token, enter it in the embedded UI as well. For an HTTPS Home Assistant
instance, use an HTTPS gateway URL so the browser does not block mixed content.
